...
#include "index_select_nvidia.cuh"

#include "../../../devices/nvidia/nvidia_kernel_common.cuh"

#include "../cuda/kernel.cuh"

namespace op::index_select::nvidia {
...
infiniStatus_t launchKernel(
    ...
    cudaMemcpyAsync(d_shape, info.shape.data(), ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_x_strides, info.x_strides.data(), ndim * sizeof(ptrdiff_t), cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_y_strides, info.y_strides.data(), ndim * sizeof(ptrdiff_t), cudaMemcpyHostToDevice, stream);
...
infiniStatus_t Descriptor::calculate(
    ...
    if (_info.atype == INFINI_DTYPE_F32) {
        return launchKernel(_info, workspace, (float *)y, (const float *)x, idx_ptr, cuda_stream);
    } else if (_info.atype == INFINI_DTYPE_F16) {
        return launchKernel(_info, workspace, (half *)y, (const half *)x, idx_ptr, cuda_stream);
    } else if (_info.atype == INFINI_DTYPE_BF16) {
        return launchKernel(_info, workspace, (__nv_bfloat16 *)y, (const __nv_bfloat16 *)x, idx_ptr, cuda_stream);
    } else if (_info.atype == INFINI_DTYPE_F64) {
        return launchKernel(_info, workspace, (double *)y, (const double *)x, idx_ptr, cuda_stream);
    }
